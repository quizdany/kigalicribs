...
export async function PATCH(request: any, context: any) {
  try {
    ...
    // verify owner
  const existingRes: any = await supabase.from('properties').select('owner_id').eq('id', id).maybeSingle()
  ...
  const { data, error } = await supabase.from('properties').update(body).eq('id', id).select().maybeSingle() as any
    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
    return NextResponse.json({ success: true, data })
  } catch (err) {
    return NextResponse.json({ success: false, error: err instanceof Error ? err.message : 'Unknown error' }, { status: 500 })
